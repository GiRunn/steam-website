...
-- 在原有代码后添加自动打开报告的函数
CREATE OR REPLACE FUNCTION review_system.open_test_report(report_path TEXT)
RETURNS void AS $$
DECLARE
    os_type TEXT;
    open_command TEXT;
BEGIN
    -- 获取操作系统类型
    os_type := CASE 
        WHEN EXISTS (SELECT 1 FROM pg_file_stat('/proc')) THEN 'LINUX'
        WHEN EXISTS (SELECT 1 FROM pg_file_stat('/Applications')) THEN 'MACOS'
        ELSE 'WINDOWS'
    END;

    -- 根据操作系统类型选择打开命令
    open_command := CASE os_type
    ...
    -- 执行打开命令
    PERFORM pg_exec(open_command);
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE '无法自动打开报告，报告路径: %', report_path;
END;
$$ LANGUAGE plpgsql;

-- 修改生成报告函数，添加自动打开功能
CREATE OR REPLACE FUNCTION review_system.generate_and_open_report()
RETURNS void AS $$
DECLARE
    report_path TEXT;
    report_content TEXT;
BEGIN
    -- 获取报告保存路径
    report_path := review_system.get_report_path();
    
    -- 生成报告内容
    report_content := review_system.generate_test_report();
    
    -- 构建完整的报告文件路径
    report_path := report_path || 'report_' || 
                  TO_CHAR(CURRENT_TIMESTAMP, 'YYYY_MM_DD_HH24_MI_SS') || '.html';
    
    -- 保存报告
    PERFORM pg_write_file(report_path, report_content, false);
    
    -- 尝试自动打开报告
    PERFORM review_system.open_test_report(report_path);
    
    -- 输出报告路径
    RAISE NOTICE '测试报告已生成: %', report_path;
END;
$$ LANGUAGE plpgsql; 