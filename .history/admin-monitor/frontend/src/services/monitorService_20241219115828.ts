...
export const getSystemMetrics = async () => {
    try {
        const response = await retryRequest(() => api.get('/system-metrics'));
...
export const getReviewSystemMetrics = async () => {
    try {
        const response = await retryRequest(() => api.get('/review-system/metrics'));
        return response.data.data || {
            total_reviews: 0,
            avg_rating: 0,
            reviews_last_hour: 0,
...
export const getSecurityEvents = async () => {
    try {
        ...
        if (response.data.code === 200) {
            ...
            throw new Error(response.data.message || '获取安全事件数据失败');
        }
    } catch (error: any) {
        if (error.response?.status === 500) {
            // 如果是服务器错误，返回友好的错误信息
