...
DECLARE
    ...
    actual_table text;
BEGIN
    -- 获取实际的父表名（通过正则表达式提取基本表名）
    IF TG_TABLE_NAME ~ '_y\d{4}m\d{2}$' THEN
        -- 如果是分区表，提取基本表名
        actual_table := (regexp_match(TG_TABLE_NAME, '^(.*?)_y\d{4}m\d{2}$'))[1];
    ELSE
        -- 如果是父表，直接使用
        actual_table := TG_TABLE_NAME;
    END IF;
    
    -- 计算分区日期(按月)
    IF actual_table = 'review_summary_partitioned' THEN
        partition_date := date_trunc('month', NEW.last_updated);
    ELSIF actual_table IN ('reviews_partitioned', 'review_replies_partitioned') THEN
        partition_date := date_trunc('month', NEW.created_at);
    ELSE
        RAISE EXCEPTION 'Unknown parent table: %', actual_table;
    END IF;
    ...
    IF NOT EXISTS (
        ...
        EXECUTE format(
            ...
            TG_TABLE_SCHEMA, partition_name, TG_TABLE_SCHEMA, actual_table,
            start_date, end_date
        );
        
        RAISE NOTICE 'Created new partition: %.% for parent table %', TG_TABLE_SCHEMA, partition_name, actual_table;
