...
const ReplyList = ({ replies = mockRepliesData }) => {
  ...
  const validReplies = useMemo(() => {
    if (!replies && !mockRepliesData) {
      console.warn('No reply data available, using empty array as fallback');
      return [];
    }
    return replies || mockRepliesData;
  }, [replies]);
  
  // 根据排序方式处理回复列表
  const sortedReplies = useMemo(() => {
    try {
      const list = [...validReplies];
      switch (sort) {
        case 'earliest':
  ...
  if (process.env.NODE_ENV === 'development') {
    console.log('ReplyList render:', {
      providedReplies: replies,
      mockData: mockRepliesData,
      validReplies,
      sortedReplies,
      currentSort: sort
    });
