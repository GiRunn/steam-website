...
-- 0. 先删除已存在的新表（如果存在）
DROP TABLE IF EXISTS review_system.reviews_partitioned_new CASCADE;
...
BEGIN
    -- 获取数据的时间范围
    SELECT 
        COALESCE(DATE_TRUNC('month', MIN(created_at)), CURRENT_DATE - INTERVAL '1 year'),
        COALESCE(DATE_TRUNC('month', MAX(created_at)), CURRENT_DATE + INTERVAL '3 months')
...
SELECT 
    review_id, game_id, user_id, rating, content, 
    playtime_hours, platform, language, is_recommended, 
    COALESCE(likes_count::INTEGER, 0), review_status, created_at, updated_at, deleted_at
FROM review_system.reviews_partitioned
