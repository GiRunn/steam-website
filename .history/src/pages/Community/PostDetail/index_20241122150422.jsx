          ...
          >
            <motion.div
              ...
              className="relative w-full max-w-md p-6 rounded-xl 
                bg-[#0a0f16] border border-white/10 shadow-xl"
            >
              <Suspense fallback={
        ...
        <AnimatePresence>
          {hasScrolled && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 20 }}
              className="px-4 py-2 rounded-lg bg-white/5 backdrop-blur-sm 
                text-sm text-gray-400 shadow-lg"
            >
              按 <kbd className="px-2 py-0.5 rounded bg-white/10">ESC</kbd> 
              返回社区
            </motion.div>
          )}
        </AnimatePresence>
