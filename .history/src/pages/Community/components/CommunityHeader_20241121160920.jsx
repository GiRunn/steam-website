...
const AnnouncementDialog = React.memo(({ announcement, isOpen, onClose }) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent 
        className="w-[95%] max-w-6xl p-0 border-none bg-[#0f1724] overflow-hidden"
      ...
      >
        ...
        <div className="relative max-h-[90vh] overflow-y-auto p-8">
          {/* 关闭按钮 */}
          <div 
            className="absolute top-6 right-6 p-2 rounded-full hover:bg-white/10 transition-colors group cursor-pointer z-10"
            onClick={onClose}
            role="button"
            tabIndex={0}
          >
          ...
          <div className="space-y-8">
            ...
            <div className="flex items-start gap-4">
              <div className="flex-shrink-0 w-14 h-14 rounded-lg bg-gradient-to-br from-blue-500 to-purple-500 p-0.5">
                <div className="w-full h-full bg-[#0f1724] rounded-[7px] flex items-center justify-center">
            ...
            {announcement.media && announcement.media.length > 0 && (
              <div className="space-y-6">
                {announcement.media.map((item, index) => (
                  <div 
                    key={index} 
                    className="rounded-xl overflow-hidden bg-black/20 shadow-lg"
                  >
                    {item.type === 'image' ? (
                      // 图片容器
                      <div className="relative">
                        ...
                        {item.alt && (
                          <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 to-transparent">
                            <p className="text-sm text-white/90 p-4">{item.alt}</p>
                          </div>
                        )}
                      </div>
                    ) : (
                      ...
                      <div className="relative aspect-video">
                        ...
                        {item.description && (
                          <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 to-transparent">
                            <p className="text-sm text-white/90 p-4">{item.description}</p>
                          </div>
                        )}
                      </div>
                    )}
