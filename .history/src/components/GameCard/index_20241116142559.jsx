...
const DynamicBackground = memo(() => {
  ...
  useEffect(() => {
    ...
    const initParticles = () => {
      particles = [];
      const numberOfParticles = Math.floor((canvas.width * canvas.height) / 20000); // 减少粒子数量以提高性能
  ...
  return (
    <>
      <canvas
        ref={canvasRef}
        className="fixed inset-0 pointer-events-none bg-[#0a0f16]"
        style={{ zIndex: -1 }}
      />
      <motion.div
        ...
        <AnimatePresence>
          {!isHovered && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="absolute top-0 inset-x-0 p-6 flex justify-between items-start z-10"
            >
    ...
    <section className="relative py-1 min-h-screen overflow-hidden">
      ...
      <div className="relative max-w-[1440px] mx-auto px-6 sm:px-8 lg:px-12">
        ...
        {filteredGames.length > 0 && !isLoading && (
          <div className="mt-16 flex justify-center items-center text-[#4d5969]">
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.2 }}
            ...
            >
              ...
              <span className="text-sm">更多游戏加载中...</span>
            </motion.div>
          </div>
        )}
