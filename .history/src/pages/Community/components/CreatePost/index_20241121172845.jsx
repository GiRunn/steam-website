...
// 在 ContentEditor 组件中修改
const getEmojiPickerPosition = () => {
  if (!editorRef.current) return {};
  
  const editorRect = editorRef.current.getBoundingClientRect();
  const windowHeight = window.innerHeight;
  const pickerHeight = 435; // emoji picker 的大致高度
  const pickerWidth = 352;  // emoji picker 的大致宽度
  const spacing = 10; // 间距

  // 检查底部空间
  const spaceBelow = windowHeight - editorRect.bottom;
  // 检查右侧空间
  const spaceRight = window.innerWidth - editorRect.right;

  // 垂直位置计算
  const showAbove = spaceBelow < (pickerHeight + spacing);
  const top = showAbove 
    ? -(pickerHeight + spacing)  // 显示在上方
    : editorRect.height + spacing; // 显示在下方

  // 水平位置计算
  const right = spaceRight < (pickerWidth + spacing)
    ? 0  // 右边空间不足，靠右对齐
    : 'auto';
  const left = spaceRight < (pickerWidth + spacing)
    ? 'auto' // 右边空间不足，使用right定位
    : 0;    // 右边空间充足，靠左对齐
