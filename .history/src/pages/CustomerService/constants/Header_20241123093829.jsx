    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          exit={{ opacity: 0, y: -10 }}
          className="fixed w-full max-w-2xl left-1/2 -translate-x-1/2 mt-2 
            bg-[#0a0f16]/95 backdrop-blur-xl rounded-xl border border-gray-800/50 
            shadow-2xl shadow-black/20 overflow-hidden z-[100]"
...
const QuickLinks = () => {
  ...
  return (
    ...
    >
      {links.map(({ icon: Icon, text, color, bgColor, hoverColor, borderColor, description }, index) => (
        <motion.button
          key={text}
          ...
          className={`relative group flex flex-col items-center p-4 rounded-xl
            ${bgColor} ${hoverColor} border ${borderColor}
        ...
        >
          ...
          {/* 装饰光效 */}
          <motion.div 
            className={`absolute inset-0 rounded-xl ${bgColor} opacity-0
