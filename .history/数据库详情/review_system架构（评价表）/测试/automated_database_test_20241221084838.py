...
class DatabaseTestFramework:
    ...
    def run_test(self, test_func, test_type: TestType, test_name: str, **kwargs) -> TestResult:
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("SET statement_timeout = '60s'")  # 全局设置60秒超时
            
            result = test_func(conn, **kwargs)
            
            # 检查是否超时
            if time.time() - start_time > 60:  # 如果测试运行超过60秒
...
class BusinessTester:
    ...
    def test_review_crud(self, conn: psycopg2.extensions.connection) -> Dict[str, Any]:
        ...
        try:
            # 添加超时控制
            cursor.execute("SET statement_timeout = '30s'")  # 设置语句超时为30秒
            ...
            cursor.execute("""
                ...
                (game_id, user_id, rating, content, playtime_hours, created_at)
                VALUES (%s, %s, %s, %s, %s, CURRENT_TIMESTAMP)
                RETURNING review_id
            """, (1001, 1, 4.5, '测试评论内容', 10))
...
class StressTester:
    ...
    def test_high_load(self, conn: psycopg2.extensions.connection) -> Dict[str, Any]:
        ...
        # 测试参数
        num_iterations = 100  # 从1000改为100
        batch_size = 20      # 从100改为20
...
class ConcurrencyTester:
    ...
    def test_concurrent_writes(self, conn: psycopg2.extensions.connection) -> Dict[str, Any]:
        ...
        """
        results = []
        num_threads = 5      # 从10改为5
        operations_per_thread = 20  # 从100改为20
...
def main():
    ...
    try:
        ...
        business_result = framework.run_test(
            business_tester.test_review_crud,
            TestType.BUSINESS,
            "评论CRUD测试"
        )
        print("\n=== 业务测试结果 ===")
        print(f"测试名称: {business_result.test_name}")
        print(f"状态: {'通过' if business_result.status else '失败'}")
        print(f"耗时: {business_result.duration:.2f}秒")
        print(f"详情: {business_result.message}")
        if business_result.details:
            print("测试详细信息:")
            for key, value in business_result.details.items():
                print(f"  {key}: {value}")

        # 执行性能测试
        logger.info("执行性能测试...")
        performance_tester = PerformanceTester(framework)
        perf_result = framework.run_test(
            performance_tester.test_query_performance,
            TestType.PERFORMANCE,
            "查询性能测试"
        )
        print("\n=== 性能测试结果 ===")
        print(f"测试名称: {perf_result.test_name}")
        print(f"状态: {'通过' if perf_result.status else '失败'}")
        print(f"耗时: {perf_result.duration:.2f}秒")
        print(f"详情: {perf_result.message}")
        if perf_result.details:
            print("测试详细信息:")
            for key, value in perf_result.details.items():
                print(f"  {key}: {value}")
        ...
        # 保存报告
        report_time = datetime.now().strftime("%Y%m%d_%H%M%S")
        report_file = f"test_report_{report_time}"
        ...
        print(f"\n测试报告已保存到: {report_file}.json 和 {report_file}.html")
