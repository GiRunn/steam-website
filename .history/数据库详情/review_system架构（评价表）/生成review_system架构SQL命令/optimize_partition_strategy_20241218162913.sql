...
BEGIN
    -- 获取数据的时间范围
    SELECT 
        COALESCE(DATE_TRUNC('month', MIN(created_at)), v_current_date),
        COALESCE(DATE_TRUNC('month', MAX(created_at)), v_current_date)
    INTO v_min_date, v_max_date
    FROM review_system.reviews_partitioned;

    -- 如果reviews_partitioned表为空，则使用当前月份作为起始点
    IF v_min_date IS NULL THEN
        v_min_date := DATE_TRUNC('month', v_current_date);
...
DECLARE
    v_current_date DATE := CURRENT_DATE;
    v_start_date DATE := DATE_TRUNC('month', v_current_date);
    v_end_date DATE := v_start_date + INTERVAL '1 month';
    v_partition_name TEXT := 'reviews_y' || TO_CHAR(v_current_date, 'YYYY') || 'm' || TO_CHAR(v_current_date, 'MM');
BEGIN
    -- 检查当前月份分区是否存在
    IF NOT EXISTS (
        SELECT 1 
