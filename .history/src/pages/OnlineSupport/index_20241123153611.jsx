...
const OnlineSupportPage = () => {
  ...
  const [quickRepliesHeight, setQuickRepliesHeight] = useState(0);
  ...
  useEffect(() => {
    ...
    const resizeObserver = new ResizeObserver(entries => {
      for (const entry of entries) {
        ...
        setQuickRepliesHeight(height);
        
        // 如果消息列表存在，调整其底部padding以适应快捷回复的高度
        if (messageListRef.current) {
          messageListRef.current.style.paddingBottom = `${height}px`;
          // 滚动到底部
        ...
        <Suspense fallback={<LoadingScreen />}>
          <div className="flex-1 overflow-hidden flex flex-col relative">
            ...
            {chatState === ChatState.CONNECTED && (
              <div 
                ref={quickRepliesRef}
                className="absolute bottom-0 left-0 right-0 w-full bg-[#0a0f16]"
                style={{
                  transform: `translateY(${quickRepliesHeight}px)`,
                  transition: 'transform 0.3s ease'
                }}
              >
                <div 
                  className="absolute bottom-full left-0 right-0 h-8 bg-gradient-to-t from-[#0a0f16] to-transparent"
                  aria-hidden="true"
                />
            ...
            {(chatState === ChatState.CONNECTED || chatState === ChatState.BOT) && (
              <div className="sticky bottom-0 left-0 right-0 bg-[#0a0f16]">
                <MessageInput 
