      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            setVisibleFeatures((prevVisibleFeatures) => [
              ...prevVisibleFeatures,
              entry.target.id,
            ]);
      ...
      {features.map((feature, index) => (
        <div
          ...
          className={`
            ...
            ${visibleFeatures.includes(feature.id) ? 'translate-y-0 opacity-100' : 'translate-y-20'}`}
          style={{ transitionDelay: `${index * 200}ms` }}
        >
          <FeatureSection
            feature={feature}
            index={index}
            inView={visibleFeatures.includes(feature.id)}
          />
        </div>
      ))}
    </div>
