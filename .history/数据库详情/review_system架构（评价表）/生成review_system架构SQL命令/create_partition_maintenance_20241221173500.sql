...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
        -- 注意: 这里修改了语法，使用正确的字符串拼接方式
        PERFORM cron.schedule(
            'partition_maintenance_job',  -- job name
            '0 2 * * *',                 -- cron schedule
            'SELECT review_system.maintain_partitions()'  -- SQL command
        );
    END IF;
END $$;

-- 添加分区性能优化
CREATE OR REPLACE FUNCTION review_system.optimize_partition_performance()
RETURNS void AS $$
DECLARE
    v_partition record;
BEGIN
    ...
    LOOP
        ...
        -- 重新组织表空间
        EXECUTE format('CLUSTER %I.%I', v_partition.schemaname, v_partition.tablename);
    END LOOP;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
        SELECT cron.schedule('0 3 * * 0', $$SELECT review_system.optimize_partition_performance()$$);
    END IF;
END $$; 