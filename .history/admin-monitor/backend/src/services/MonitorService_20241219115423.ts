...
class MonitorService {
            ...
            const result = await client.query(`
                WITH partition_info AS (
                    SELECT 
                        ...
                        CASE 
                            WHEN pg_stat_get_live_tuples(c.oid) = 0 THEN 0
                            ELSE ROUND(100.0 * pg_stat_get_dead_tuples(c.oid) / 
                                NULLIF(pg_stat_get_live_tuples(c.oid) + pg_stat_get_dead_tuples(c.oid), 0), 2)
                        END as dead_tuple_percent
                    FROM pg_class c
                    JOIN pg_namespace n ON n.oid = c.relnamespace
                    WHERE n.nspname = 'review_system'
                    AND c.relkind = 'r'  -- 只选择普通表
                    AND c.relname ~ '_y\\d{4}m\\d{2}$'  -- 匹配分区表命名模式
                )
                SELECT 
                    schema_name || '.' || partition_name as table_name,
                    partition_name,
                    size,
                    size_bytes,
                    row_count,
    ...
    private extractDateFromPartitionName(partitionName: string): string {
