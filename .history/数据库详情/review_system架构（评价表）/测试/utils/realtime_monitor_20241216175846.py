    ...
    def initialize_monitoring(self):
        ...
        try:
            ...
            if not os.path.exists(setup_file):
                print(f"{Fore.RED}错误: 找不到设置脚本文件 {setup_file}{Style.RESET_ALL}")
                sys.exit(1)
            
            with open(setup_file, 'r', encoding='utf-8') as f:
                setup_sql = f.read()
                
                # 使用正则表达式分割SQL语句，保留美元符号引用
                import re
                statements = re.split(r';(?=[^\$]*(?:\$[^\$]*\$[^\$]*)*$)', setup_sql)
                ...
                for statement in statements:
                    if statement.strip():
                        try:
                            ...
                            print(f"{Fore.RED}执行SQL语句失败: {str(e)}{Style.RESET_ALL}")
                            print(f"问题语句长度: {len(statement.strip())}")
                            # 只打印前200个字符，避免输出过长
                            print(f"问题语句开始部分: {statement.strip()[:200]}...")
                            raise
