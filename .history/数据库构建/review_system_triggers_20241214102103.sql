...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        AND c.relname = TG_TABLE_NAME || '_' || partition_name
    ) THEN
        -- 创建对应的分区
        EXECUTE format(
            'CREATE TABLE review_system.%I PARTITION OF review_system.%I
             FOR VALUES FROM (%L) TO (%L)',
            TG_TABLE_NAME || '_' || partition_name,
            TG_TABLE_NAME,
            start_date,
            end_date
        );
        
        RAISE NOTICE '已自动创建分区: %', TG_TABLE_NAME || '_' || partition_name;
    END IF;
