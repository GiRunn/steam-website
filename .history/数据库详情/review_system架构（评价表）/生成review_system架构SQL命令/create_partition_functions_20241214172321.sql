...
BEGIN
    ...
    FOR i IN 0..months_ahead LOOP
        ...
        IF NOT EXISTS (
            ...
            -- 记录分区创建
            PERFORM review_system.record_partition_creation(
                'reviews_partitioned',
                partition_name,
                start_date,
                end_date
...
-- 创建每日检查触发器函数
CREATE OR REPLACE FUNCTION review_system.check_partition_trigger()
RETURNS TRIGGER AS $$
BEGIN
    -- 如果是每月30日，检查分区状态
    IF EXTRACT(DAY FROM NEW.created_at) = 30 THEN
        PERFORM review_system.check_partition_status();
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql; 