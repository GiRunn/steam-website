...
const Homepage = () => {
  ...
  const FeaturesContainer = ({ features }) => {
    // 使用 useRef 存储已动画的状态，避免重渲染
    const animatedFeaturesRef = useRef(new Set());
      ...
      const observer = new IntersectionObserver(
        (entries) => {
          entries.forEach((entry) => {
            ...
            // 只在首次进入视口时触发
            if (featureId && entry.isIntersecting && !animatedFeaturesRef.current.has(featureId)) {
              animatedFeaturesRef.current.add(featureId);
    ...
    return (
      <div ref={featuresRef} className="mt-20 space-y-20">
        {features.map((feature, index) => (
          <div
            key={feature.id}
            data-feature-id={feature.id}
            className={`
              transform transition-opacity transition-transform duration-1000
              ${visibleFeatures[feature.id] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20'}
            `}
          >
            <FeatureSection 
              feature={feature}
