...
class DatabaseTestFramework:
    ...
    def run_test(self, test_func, test_type: TestType, test_name: str, **kwargs) -> TestResult:
        ...
        if hasattr(self, 'result_dir'):
            self.save_test_result(test_result, self.result_dir)
    ...
    # 在 DatabaseTestFramework 类中添加新方法
    def save_test_result(self, test_result: TestResult, result_dir: str) -> None:
        """保存单个测试结果到独立的JSON文件
        ...
        """
        # 创建测试结果目录
        test_results_dir = os.path.join(result_dir, "测试结果")
        os.makedirs(test_results_dir, exist_ok=True)
        
        # 根据测试类型创建子目录
        type_dir = os.path.join(test_results_dir, test_result.test_type.value)
        os.makedirs(type_dir, exist_ok=True)
        
        # 生成文件名
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"{test_result.test_name}_{timestamp}.json"
        filepath = os.path.join(type_dir, filename)
        
        # 准备测试结果数据
        result_data = {
            "测试类型": test_result.test_type.value,
            "测试名称": test_result.test_name,
            "测试状态": "通过" if test_result.status else "失败",
            "测试时间": timestamp,
            "执行时长": f"{test_result.duration:.2f}秒",
            "测试信息": test_result.message,
            "测试详情": test_result.details
        }
        ...
        try:
            ...
            self.logger.info(f"已保存测试结果到: {filepath}")
        except Exception as e:
...
class SecurityTester:
    ...
    def test_sql_injection(self, conn: psycopg2.extensions.connection) -> Dict[str, Any]:
        ...
        batch_injection_tests = [
            ...
            {
                "name": "批量更新注入",
                "data": [(1, "normal"), (2, "'); DELETE FROM users; --")]
            }
        ]
...
def main():
    ...
    try:
        ...
        # 设置结果目录
        framework.result_dir = result_dir
        ...
        # 为每种测试类型创建目录
        for test_type in TestType:
            os.makedirs(os.path.join(test_results_dir, test_type.value), exist_ok=True)
        
        # 保存每个测试结果
        for test_result in framework.test_results:
            framework.save_test_result(test_result, result_dir)
        
        # 生成汇总报告
        report = framework.generate_report()
        
        # 保存汇总报告
        summary_dir = os.path.join(result_dir, "汇总报告")
        os.makedirs(summary_dir, exist_ok=True)
        
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        framework.save_report(
            report, 
            os.path.join(summary_dir, f"测试汇总报告_{timestamp}.json")
        )
        generate_html_report(
            report, 
            os.path.join(summary_dir, f"测试汇总报告_{timestamp}.html")
        )
        
        logger.info(f"测试完成! 报告已保存到: {summary_dir}")
