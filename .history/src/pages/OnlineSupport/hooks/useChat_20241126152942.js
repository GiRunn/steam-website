  ...
  const handleSendMessage = useCallback(async (messageData) => {
    ...
    try {
      ...
      if (isQuestionSelect && question) {
        setSelectedQuestion(question);  // 只设置选择的问题，不添加消息
        return; // 直接返回，不做任何消息处理
      }
      
      // 2. 处理具体问题选择（二级菜单）
      if (isAnswerSelect && selectedQuestion && answer) {
        // 这里创建一条完整的问答消息，包含一级和二级选择
        const userMessage = {
          id: Date.now(),
          content: `${selectedQuestion.text} - ${answer.text}`,
          isUser: true,
          timestamp: new Date().toISOString(),
        ...
        const botResponse = {
          id: Date.now() + 1,
          content: getBotResponseForAnswer(selectedQuestion.id, answer.text),
          isBot: true,
          timestamp: new Date().toISOString()
        };
