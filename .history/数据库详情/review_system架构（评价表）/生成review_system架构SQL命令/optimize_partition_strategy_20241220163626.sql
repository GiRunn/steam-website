...
-- 1. 删除已存在的索引（如果存在）
DROP INDEX IF EXISTS review_system.idx_reviews_game_rating;
DROP INDEX IF EXISTS review_system.idx_reviews_created_at_brin;

-- 2. 创建新的优化索引
CREATE INDEX IF NOT EXISTS idx_reviews_game_rating ON review_system.reviews_partitioned
    (game_id, rating)
    INCLUDE (user_id, created_at)
    WHERE review_status = 'active';

CREATE INDEX IF NOT EXISTS idx_reviews_created_at_brin ON review_system.reviews_partitioned 
    USING BRIN (created_at) WITH (pages_per_range = 128);

-- 3. 优化现有索引的填充因子
ALTER INDEX review_system.idx_reviews_game_rating SET (fillfactor = 90);

-- 4. 添加统计信息
ANALYZE review_system.reviews_partitioned;

-- 5. 创建索引维护函数
CREATE OR REPLACE FUNCTION review_system.maintain_indexes()
RETURNS void AS $$
BEGIN
    -- 重建碎片化的索引
    REINDEX INDEX CONCURRENTLY review_system.idx_reviews_game_rating;
    REINDEX INDEX CONCURRENTLY review_system.idx_reviews_created_at_brin;
...
-- 6. 创建定期维护任务
CREATE OR REPLACE FUNCTION review_system.schedule_index_maintenance()
RETURNS void AS $$
BEGIN
    -- 每天凌晨3点执行维护
    SELECT cron.schedule('0 3 * * *', 'SELECT review_system.maintain_indexes()');
END;
$$ LANGUAGE plpgsql;