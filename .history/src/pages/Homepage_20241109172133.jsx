...
const Homepage = () => {
  ...
  useEffect(() => {
    ...
    const scrollContainer = brandContainerRef.current;
    ...
    const scroll = () => {
      setScrollPosition((prevPosition) => {
        ...
        // 当滚动到最后一个元素后重置位置
        if (newPosition > speed * brandLogos.length * 100) {
          return 0;
        }
        return newPosition;
      });
  ...
  const handleMouseMove = (e) => {
    ...
    setScrollPosition(prev => prev - walk/50);
  };
 ...
 // 添加品牌数据
  ...
  return (
    <div className="absolute inset-0 bg-gradient-to-b from-[#1b2838] via-[#1b2838] to-[#1b2838]">
      ...
      {showVideo && (
        <div 
          className="fixed inset-0 bg-black bg-opacity-75 z-75 flex items-center justify-center"
      ...
      <div className="brand-showcase relative py-20 overflow-hidden bg-[#1b2838]">
          {/* 背景装饰 */}
          <div className="absolute inset-0">
          ...
          <div className="relative w-full overflow-hidden">
            ...
            >
              ...
              {[...brandLogos, ...brandLogos, ...brandLogos].map((brand, index) => (
                <div
                  key={`${brand.id}-${index}`}
                  className="brand-logo relative group flex-shrink-0 mx-8"
                >
                  <div className="relative w-48 h-20">
                    <img
                      src={brand.image}
                      alt={brand.name}
                      className="w-full h-full object-contain opacity-50 group-hover:opacity-100 
                        transition-all duration-500 filter grayscale group-hover:grayscale-0"
                      draggable="false"
                    />
                    
                    {/* 悬浮光效 */}
                    <div className="absolute inset-0 opacity-0 group-hover:opacity-100 
                      transition-opacity duration-500 bg-blue-500/10 rounded-lg" />
                    
                    {/* 底部光线 */}
                    <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-3/4 h-1 
                      opacity-0 group-hover:opacity-100 transition-all duration-500 
                      bg-gradient-to-r from-transparent via-blue-500 to-transparent blur-sm" />
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* 渐变遮罩 */}
          <div className="absolute top-0 left-0 w-32 h-full bg-gradient-to-r from-[#1b2838] to-transparent pointer-events-none" />
          <div className="absolute top-0 right-0 w-32 h-full bg-gradient-to-l from-[#1b2838] to-transparent pointer-events-none" />
        </div>
