...
-- 5. 创建初始分区（2024年12月和2025年1月）
DO $$
BEGIN
    -- 创建2024年12月分区
    PERFORM review_system.auto_create_partition(
        ROW('2024-12-01 00:00:00+08'::timestamp with time zone)::review_system.reviews_partitioned
    );
    
    -- 创建2025年1月分区
    PERFORM review_system.auto_create_partition(
        ROW('2025-01-01 00:00:00+08'::timestamp with time zone)::review_system.reviews_partitioned
    );
END $$; 