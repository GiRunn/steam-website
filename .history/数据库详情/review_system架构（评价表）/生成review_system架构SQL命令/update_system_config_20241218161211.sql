...
BEGIN
    ...
    IF v_os_type = 'Windows' THEN
        -- Windows平台特定配置
        EXECUTE 'ALTER SYSTEM SET effective_io_concurrency = 0';  -- Windows不支持posix_fadvise
        EXECUTE 'ALTER SYSTEM SET shared_buffers = ''1GB''';        -- Windows建议值
        EXECUTE 'ALTER SYSTEM SET effective_cache_size = ''3GB''';  -- Windows建议值
    ELSE
        -- Linux/Unix平台配置
    ...
    -- 并发配置
    EXECUTE 'ALTER SYSTEM SET max_connections = 100';             -- 降低以适应Windows
    EXECUTE 'ALTER SYSTEM SET max_parallel_workers_per_gather = 2';
    EXECUTE 'ALTER SYSTEM SET max_parallel_workers = 4';
    
    -- 缓存行为配置
    EXECUTE 'ALTER SYSTEM SET synchronous_commit = ''on''';         -- 为安全起见改为on
    EXECUTE 'ALTER SYSTEM SET full_page_writes = ''on''';          -- 为安全起见改为on
    
    -- 统计信息收集
    EXECUTE 'ALTER SYSTEM SET default_statistics_target = 500';   -- 降低以减少内存使用
