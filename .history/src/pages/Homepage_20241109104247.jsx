...
const Homepage = () => {
  ...
  // 在FeaturesContainer组件中
  const FeaturesContainer = ({ features }) => {
    // 滚动事件处理函数
    const handleScroll = throttle(() => {
      const featuresTop = featuresRef.current.getBoundingClientRect().top;
      const triggerPoint = window.innerHeight * 0.8; // 触发点为视口高度的80%
      
      if (featuresTop < triggerPoint) {
        setIsVisible(true);
      } else {
        setIsVisible(false);
      }
    ...
    return (
      <div 
        ref={featuresRef}
        className={`mt-20 space-y-20 ${isVisible ? 'opacity-100' : 'opacity-0'}`}
      >
        {features.map((feature, index) => (
          <div
            ...
            className={`
              ...
              ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-20'}`}
