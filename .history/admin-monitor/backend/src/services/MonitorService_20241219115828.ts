            ...
            const sysMetricsQuery = `
                WITH system_stats AS (
                    SELECT 
                        ...
                        -- 内存使用率计算 (基于shared_buffers和work_mem设置)
                        (SELECT ROUND(
                            (current_setting('shared_buffers')::bigint * 
                            (SELECT setting::bigint FROM pg_settings WHERE name = 'block_size')) * 100.0 /
                            (SELECT CAST(NULLIF(REGEXP_REPLACE(setting, '[^0-9]', '', 'g'), '') AS bigint) * 1024 * 1024
                            FROM pg_settings WHERE name = 'effective_cache_size'), 2)
                        ) as memory_usage,
