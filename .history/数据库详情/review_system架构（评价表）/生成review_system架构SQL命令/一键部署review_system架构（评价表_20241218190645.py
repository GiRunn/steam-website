...
def handle_port_5432():
    ...
    try:
        ...
        for pid, name in processes.items():
            ...
            if kill_process_safely(pid, name, force=force_kill):
                print(f"✓ 已{'强制' if force_kill else ''}结束进程: {name} (PID: {pid})")
                success = True
            else:
