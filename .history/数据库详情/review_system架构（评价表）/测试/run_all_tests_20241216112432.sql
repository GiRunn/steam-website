...
BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
        ROLLBACK TO basic_tests;
        RAISE NOTICE '基础数据测试失败: %', SQLERRM;
    END;

    -- 分区测试
    SAVEPOINT partition_tests;
    BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
        ROLLBACK TO trigger_tests;
        RAISE NOTICE '触发器测试失败: %', SQLERRM;
    END;

    -- 性能测试
    SAVEPOINT performance_tests;
    BEGIN
