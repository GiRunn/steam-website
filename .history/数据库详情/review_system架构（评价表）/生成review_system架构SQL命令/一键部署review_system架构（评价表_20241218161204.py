...
def execute_system_config(conn, sql_file):
    ...
    try:
        with conn.cursor() as cur:
            # 先执行DO语句块
            do_block = sql_file[sql_file.find('DO $$'):sql_file.find('END $$;') + 7]
            if do_block:
                try:
                    cur.execute(do_block)
                    conn.commit()
                    logging.info("系统配置DO块执行成功")
                except Exception as e:
                    logging.error(f"执行系统配置DO块失败: {str(e)}")
                    return False

            # 执行其余的配置语句
            remaining_sql = sql_file[sql_file.find('END $$;') + 7:]
            statements = [stmt.strip() for stmt in remaining_sql.split(';') 
            ...
            for stmt in statements:
                try:
                    cur.execute(stmt)
                    conn.commit()
                    if 'pg_reload_conf()' in stmt:
                        logging.info("配置重新加载成功")
                except Exception as e:
                    if "warning" in str(e).lower():
                        logging.warning(f"参数设置警告: {str(e)}")
                        continue
                    logging.error(f"执行系统配置语句失败: {stmt}\n错误: {str(e)}")
                    return False
            ...
            try:
                ...
                for row in cur.fetchall():
                    logging.info(f"当前参数值 - {row[0]}: {row[1]}{row[2] if row[2] else ''}")
            except Exception as e:
