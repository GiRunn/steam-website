...
class MonitorService {
    ...
    async getMetricsHistory(metricType: string, timeRange: string) {
        const client = await pool.connect();
        try {
            ...
            switch(metricType) {
                case 'cpu':
                    tableName = 'cpu_usage_history';
                    columns = 'usage_percent as value';
                    break;
                case 'memory':
                    tableName = 'memory_usage_history';
                    columns = 'used_memory as value';
                    break;
                case 'connections':
                    tableName = 'connection_history';
                    columns = 'total_connections as value';
                    break;
                case 'transactions':
                    tableName = 'transaction_history';
                    columns = 'tps as value';
                    break;
                default:
            ...
            const result = await client.query(`
                SELECT 
                    time_bucket($1::interval, collected_at) AS time,
                    avg(${columns})::numeric(10,2) as value,
                    database_name
                FROM review_system.${tableName}
                WHERE 
                    collected_at >= NOW() - $2::interval
                    AND database_name = current_database()
                GROUP BY time_bucket($1::interval, collected_at), database_name
                ORDER BY time
            `, [interval, timeRange]);
