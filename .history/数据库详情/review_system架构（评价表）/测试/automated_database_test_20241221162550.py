...
class DatabaseTestFramework:
    ...
    def create_adverse_conditions(self, conn: psycopg2.extensions.connection) -> Dict[str, Any]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            future = executor.submit(self._test_connection_flood)
            try:
                connection_results = future.result(timeout=30)  # 30秒超时
        ...
        try:
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(self._test_data_corruption, conn)
                corruption_results = future.result(timeout=20)  # 20秒超时
        ...
        try:
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(self._test_network_failures)
                network_results = future.result(timeout=20)  # 20秒超时
        ...
        return {
            ...
            "summary": {
                ...
                "completed_tests": sum(1 for r in results if r.get("status") not in ["超时", "错误"]),
                "timeout_tests": sum(1 for r in results if r.get("status") == "超时"),
                "error_tests": sum(1 for r in results if r.get("status") == "错误")
            }
        }
    ...
    def _test_connection_flood(self) -> Dict[str, Any]:
        ...
        connections = []
        errors = []
        max_test_connections = 20  # 减少测试连接数
        connection_timeout = 2     # ��少连接超时时间
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=5) as executor:
                ...
                for _ in range(max_test_connections):
                    futures.append(executor.submit(create_connection))
...
class SecurityTester:
    ...
    def test_sql_injection(self, conn: psycopg2.extensions.connection) -> Dict[str, Any]:
        ...
        # 添加超时控制
        cursor.execute("SET statement_timeout = '5s'")  # 设置语句超时为5秒
        
        # 修改注入测试模式,移除可能导致长时间执行的模式
        injection_patterns = [
            {
                "name": "基础布尔注入",
                "pattern": "' OR '1'='1",
        ...
        for test_case in injection_patterns:
            try:
                # 添加查询超时控制
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(self._execute_injection_test, cursor, test_case)
                    try:
    ...
    def _execute_injection_test(self, cursor, test_case) -> Dict[str, Any]:
        """执行单个注入测试"""
        start_time = time.time()
        try:
            unsafe_query = f"""
    ...
    def test_review_crud(self, conn: psycopg2.extensions.connection) -> Dict[str, Any]:
        ...
        try:
            ...
            results.append({
                "operation": "CREATE",
                "status": "通过",
                "message": f"成���创建{len(test_data)}条测试评论",
                "test_data": [
                    {
                        "review_id": row[0],
                        "game_id": row[1],
...
class PartitionTester:
    ...
    def test_partition_creation(self, conn: psycopg2.extensions.connection) -> Dict[str, Any]:
        ...
        try:
            ...
            partitions = cursor.fetchall()
            self.logger.info(f"成���创建 {len(partitions)} 个分区")
...
def main():
    ...
    try:
        ...
        # 修改测试执行部分,添加超时控制
        def run_test_with_timeout(test_func, *args, timeout=300):
            """使用超时控制运行测试"""
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(test_func, *args)
                try:
                    return future.result(timeout=timeout)
                except concurrent.futures.TimeoutError:
                    logger.error(f"{test_func.__name__} 测试超时")
                    return None
        
        # 执行测试时使用超时控制
        for test_type, test_func, test_name in [
            (TestType.SECURITY, security_tester.test_sql_injection, "SQL注入测试"),
            (TestType.SECURITY, security_tester.test_access_control, "访问控制测试"),
            # ... 其他测试 ...
        ]:
            ...
            if result is None:
                logger.warning(f"{test_name} 执行超时,跳过该测试")
                continue
