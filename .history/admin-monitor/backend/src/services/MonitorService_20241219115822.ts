            ...
            const sysMetricsQuery = `
                WITH system_stats AS (
                    SELECT 
                        ...
                        FROM pg_stat_activity WHERE state = 'active') as cpu_usage,
                        
                        -- 内存使用率计算
                        (SELECT ROUND(CAST(sum(work_mem) AS numeric) * 100 / 
                            (SELECT setting::numeric * 1024 FROM pg_settings WHERE name = 'max_connections'), 2)
                        FROM pg_stat_activity) as memory_usage,
