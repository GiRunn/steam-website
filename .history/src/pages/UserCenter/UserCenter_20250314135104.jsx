...
const UserCenter = () => {
  ...
  // 共享的动画配置
  const sharedLayoutTransition = theme.shared.animation.spring;

  return (
    <LayoutGroup id="user-center-layout">
      <motion.div 
        className={`min-h-screen ${theme.background.primary} transition-colors duration-300 text-sm sm:text-base font-['Poppins',system-ui,sans-serif]`}
        initial={false}
        layout
      ...
      >
        ...
        <motion.div
          ...
          transition={sharedLayoutTransition}
        >
          <Footer darkMode={isDarkMode} theme={theme} showScrollTop={showScrollTop} />
        </motion.div>
      </motion.div>
    </LayoutGroup>
  );
