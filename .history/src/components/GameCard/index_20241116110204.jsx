    ...
    >
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          <h2 className="text-xl font-medium text-[#e6e7e8]">
            探索游戏
            <div className="h-[2px] w-12 bg-[#3f85e7] mt-2" />
          </h2>
          
          {/* 搜索框部分 */}
          <div className="relative group">
            {/* 搜索框聚焦时的光效 */}
            <div className="absolute -inset-0.5 bg-[#3f85e7] opacity-0 
                          group-hover:opacity-5 blur-lg transition-all duration-300" />
            ...
            <div className="relative flex items-center">
              ...
              <input
                type="text"
                placeholder="搜索游戏..."
                className="w-64 h-9 pl-10 pr-4 bg-[#141c28] rounded text-[#e6e7e8] 
      ...
      <div className="flex items-center gap-2 overflow-x-auto hide-scrollbar">
        {GAME_FILTERS.map(filter => (
          <motion.button
            ...
            className={`
              min-w-fit px-4 py-1.5 rounded text-sm font-medium
              transition-all duration-300 select-none
              ${selectedFilter === filter.id 
                ? 'bg-[#3f85e7] text-[#e6e7e8]' 
                : 'bg-[#141c28] text-[#4d5969] hover:bg-[#1b2736] hover:text-[#8f98a0]'}
            `}
      ...
      {/* 可选：添加一个分割线 */}
      <div className="h-[1px] bg-[#1b2736] mt-2" />
    </motion.div>
  );
});
