...
const TabButton = ({ active, icon, label, onClick }) => (
  <motion.button
    onClick={onClick}
    className={`relative px-8 py-3 rounded-2xl flex items-center gap-3 transition-all
      ${active 
        ? 'bg-gradient-to-r from-violet-600/90 to-violet-700/90 text-white shadow-lg shadow-violet-500/20' 
        : 'text-violet-300/70 hover:text-violet-200 hover:bg-violet-600/10'
      }`}
  ...
  >
    {active && (
      <motion.div
        layoutId="activeTab"
        className="absolute inset-0 rounded-2xl bg-gradient-to-r from-violet-400/20 to-violet-600/20"
        initial={false}
...
{showEmptyState ? (
                <div className="col-span-full flex flex-col items-center justify-center py-16 text-gray-400">
                  <Gift className="w-16 h-16 mb-6 text-fuchsia-400" />
                  <p className="text-xl font-medium text-gray-300">该活动暂无参与商品</p>
                </div>
              ) : (
        ...
        {selectedImage && (
          ...
          >
            <motion.div
              ...
              className="relative max-w-5xl w-full mx-6 p-6 rounded-3xl overflow-hidden bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900"
