...
def execute_sql_file(cur, conn, file_path, category=None):
    ...
    try:
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            # 使用监控包装执行测试
            monitor_sql = f"""
            SELECT review_system.run_test_with_monitoring(
            ...
            # 获取监控报告
            monitoring_report = cur.fetchone()[0]
...
def run_tests():
    try:
        ...
        # 首先执行监控系统设置
        print("\n设置监控系统...")
        monitoring_setup_files = [
            os.path.join(script_dir, "utils", "enhanced_monitoring.sql"),
            os.path.join(script_dir, "utils", "test_wrapper.sql")
        ]
        
        for setup_file in monitoring_setup_files:
            if os.path.exists(setup_file):
                execute_sql_file(cur, conn, setup_file)
            else:
                print(f"错误: 找不到监控设置文件 {setup_file}")
                raise FileNotFoundError(f"监控设置文件不存在: {setup_file}")
        ...
        if os.path.exists(deploy_script):
            ...
            print(f"错误: 找不到一键部署脚本 {deploy_script}")
            raise FileNotFoundError(f"一键部署脚本不存在: {deploy_script}")
        ...
        try:
            print("\n生成总体监控报告...")
            cur.execute("SELECT review_system.generate_monitoring_report(NULL)")
            overall_report = cur.fetchone()[0]
            print("\n总体监控报告:")
            print("=" * 50)
            print(overall_report)
            print("=" * 50)
        except Exception as e:
            print(f"生成总体监控报告失败: {str(e)}")
