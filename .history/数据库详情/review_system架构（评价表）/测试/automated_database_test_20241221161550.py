        ...
        injection_patterns = [
            {
                "name": "基础SQL注入",
                "pattern": "' OR '1'='1",
                "description": "尝试绕过WHERE条件"
            },
            {
                "name": "堆叠查询注入",
                "pattern": "'; DROP TABLE test_table; --",
                "description": "尝试执行多条SQL语句"
            },
            {
                "name": "时间盲注",
            ...
            },
            {
                ...
                "description": "尝试获取敏感数据"
            },
            {
                "name": "错误注入",
                "pattern": "' AND 1=CAST((SELECT version()) AS INTEGER)--",
                "description": "利用错误消息获取信息"
            }
        ]
        
        print(f"\n[{datetime.now()}] 开始执行SQL注入测试...")
        
        for test in injection_patterns:
            try:
                print(f"\n[{datetime.now()}] 测试: {test['name']}")
                print(f"描述: {test['description']}")
                print(f"注入模式: {test['pattern']}")
                ...
                is_vulnerable = len(rows) > 0 or execution_time > 1.0
                
                status = "失败" if is_vulnerable else "安全"
                message = "发现漏洞" if is_vulnerable else "注入被阻止"
                ...
                conn.rollback()  # 确保回滚任何更改
        
        print(f"\n[{datetime.now()}] SQL注入测试完成")
        
        return {
            "test": "SQL注入测试",
            "details": {
                "测试说明": "验证系统对SQL注入的防护能力",
                "测试结果": results,
                "总结": {
                    "总测试数": len(injection_patterns),
                    "安全测试数": sum(1 for r in results if r["status"] == "安全"),
                    "发现漏洞数": sum(1 for r in results if r["status"] == "失败"),
                    "测试错误数": sum(1 for r in results if r["status"] == "错误")
                }
            }
        }
