...
const MediaGallery = ({ data, activityId }) => {
  ...
  const getActivityProducts = () => {
    ...
    // 检查 MOCK_GAMES 是否存在且为数组
    if (!Array.isArray(MOCK_GAMES)) {
      console.error('MOCK_GAMES is not an array');
      return [];
    }
  
    // 转换 activityId 为数字类型进行比较
    const targetActivityId = Number(activityId);
    const filteredGames = MOCK_GAMES.filter(game => 
      game && Number(game.activityId) === targetActivityId
    );
  
    console.log('ActivityId:', targetActivityId);
