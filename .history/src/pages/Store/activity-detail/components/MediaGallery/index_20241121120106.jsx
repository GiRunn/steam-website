...
import React, { useState } from 'react'; import { motion, AnimatePresence } from 'framer-motion'; import { Link } from 'react-router-dom'; import { Gift, Ticket, LayoutGrid, X, Calendar, Us
pasted
Z
你检查这个代码,我改为/store/game/${game.id}后报错[{
    "resource": "/e:/Steam/steam-website/src/pages/store/activity-detail/components/MediaGallery/index.jsx",
    "owner": "eslint",
    "code": {
        "value": "no-undef",
        "target": {
...
明白了，问题出在 ProductCard 组件中。您提供的代码里，ProductCard 组件接收的是 item 作为 props，但在使用时却使用了未定义的 game 变量。让我们修复这个问题：

jsx

Copy
// 修改 ProductCard 组件的代码
const ProductCard = ({ item }) => {
  return (
    // 将 game.id 改为 item.id
