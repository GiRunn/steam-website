...
COPY (
        ...
        (
            SELECT string_agg(
                format(
                    ...
                    r.test_name,
                    r.test_description,
                    CASE WHEN r.status = 'PASSED' THEN 'passed' ELSE 'failed' END,
                    r.status,
                    r.start_time,
                    r.duration,
                    CASE WHEN r.error_message IS NOT NULL 
                         THEN format('<p class="failed">错误: %s</p>', r.error_message)
                         ELSE ''
                    END,
                    COALESCE(r.details::text, '无')
                ),
                E'\n'
            )
            FROM test_results r
    ...
    FROM test_summary s
) TO 'E:\Steam\steam-website\test_report.html';

-- 显示测试完成信息
DO $$
BEGIN
    RAISE NOTICE '测试报告已生成到: E:\Steam\steam-website\test_report.html';
END $$;