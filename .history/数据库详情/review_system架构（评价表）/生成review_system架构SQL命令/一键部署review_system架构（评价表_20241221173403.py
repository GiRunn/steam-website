...
def install_pg_cron():
    ...
    try:
        ...
        if system == 'windows':
            ...
            # 1. 下载pg_cron
            download_url = f"https://github.com/citusdata/pg_cron/releases/latest/download/pg_cron_win_{postgres_version}.zip"
            ...
            if pg_config_cmd.returncode != 0:
                raise Exception("无法找到PostgreSQL安装目录")
            ...
            pg_conf = os.path.join(os.path.dirname(pg_bin), 'data', 'postgresql.conf')
            
            # 备份配置文件
            shutil.copy2(pg_conf, pg_conf + '.bak')
