...
def execute_sql_file(cur, conn, file_path, category=None, is_init=False):
    """执行单个SQL文件并处理事务"""
    try:
        print(f"\n{'='*80}")
            ...
            try:
                cur.execute("""
                    SELECT 
                        ...
                        (SELECT COALESCE(pg_size_pretty(pg_total_relation_size('review_system.reviews_partitioned')), '0 bytes')),
                        (SELECT COALESCE((SELECT count(*) FROM review_system.reviews_partitioned), 0))
                """)
                ...
                initial_stats = (db_size, connections, '0 bytes', 0)
        else:
            # 对于初始化文件，只记录简单统计
            start_time = time.time()
            initial_stats = None
...
def run_tests():
    try:
        ...
        if os.path.exists(init_file):
            print("\n初始化数据库...")
            execute_sql_file(cur, conn, init_file, is_init=True)
            conn.commit()  # 确保初始化完成
        ...
        # 修改基础架构文件的执行顺序
        base_files = [
            "create_review_tables.sql",      # 首先创建表
