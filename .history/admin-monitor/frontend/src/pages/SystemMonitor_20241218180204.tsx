...
const ReviewSystemMonitor: React.FC = () => {
    ...
    const [reviewAnomalies, setReviewAnomalies] = useState([]);

    useEffect(() => {
        const fetchReviewSystemData = async () => {
            const metrics = await getReviewSystemMetrics();
            const anomalies = await getReviewSystemAnomalies();
            setReviewMetrics(metrics);
            setReviewAnomalies(anomalies);
        };

        fetchReviewSystemData();
        const intervalId = setInterval(fetchReviewSystemData, 60000); // 每分钟刷新
    ...
    return (
        <div>
            ...
            {reviewMetrics && (
                <div>
                    <p>总评论数: {reviewMetrics.total_reviews}</p>
                    <p>平均评分: {reviewMetrics.avg_rating}</p>
                    <p>最近1小时评论数: {reviewMetrics.reviews_last_hour}</p>
                    <p>独特游戏数: {reviewMetrics.unique_games_reviewed}</p>
                    <p>平均游戏时长: {reviewMetrics.avg_playtime} 小时</p>
                </div>
            )}
            ...
            <table>
                ...
                <tbody>
                    {reviewAnomalies.map((anomaly, index) => (
                        <tr key={index}>
                            <td>{anomaly.game_id}</td>
                            <td>{anomaly.rating}</td>
                            <td>{anomaly.review_count}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
