...
const Homepage = () => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // 当滚动到 80% 时触发彩蛋
      if (scrollDepth > 0.8 && !hasTriggeredEasterEgg) {
        // 添加浮动元素
        const egg = document.createElement('div');
        egg.className = 'fixed bottom-20 right-20 animate-float';
        egg.innerHTML = '🎮';
        egg.style.fontSize = '3rem';
        document.body.appendChild(egg);
        
        // 点击彩蛋触发效果
        egg.addEventListener('click', () => {
          egg.classList.add('animate-spin');
          setTimeout(() => {
            egg.remove();
          }, 1000);
        });
