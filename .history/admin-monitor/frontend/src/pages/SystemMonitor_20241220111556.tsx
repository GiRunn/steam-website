    ...
    useEffect(() => {
        const fetchReviewStats = async () => {
            try {
                ...
                // 修改数据验证逻辑
                // 由于 axios 拦截器可能已经处理了响应，这里直接使用 response
                if (response && response.code === 200 && response.data) {
                    setReviewStats(response.data);
                    console.log('成功获取评论统计:', response.data);
                } else {
                ...
                if (isAxiosError(err)) {
                    if (err.response) {
                        ...
                        if (err.response.data && err.response.data.data) {
                            ...
                            setError(`服务器错误: ${err.response.status} - ${JSON.stringify(err.response.data)}`);
                        }
                    } else if (err.request) {
                        // 请求已发送但没有收到响应
