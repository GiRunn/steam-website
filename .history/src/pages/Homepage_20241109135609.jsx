    ...
    useEffect(() => {
      const handleScroll = () => {
        if (featuresRef.current) {
          const { top, bottom } = featuresRef.current.getBoundingClientRect();
    ...
    return (
      <div ref={featuresRef} className="mt-20 space-y-20">
        {features.map((feature, index) => (
          <div
            key={feature.id}
            className={`feature-item transform transition-all duration-1000 ${
              triggered ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'
            }`}
