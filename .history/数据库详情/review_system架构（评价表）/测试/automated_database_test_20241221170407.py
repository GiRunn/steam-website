...
class SecurityTester:
    ...
    def test_sql_injection(self, conn: psycopg2.extensions.connection) -> Dict[str, Any]:
        ...
        def classify_error(error_msg: str) -> str:
            """对错误信息进行分类"""
            if "无效的类型" in error_msg:
                return "数据类型验证阻止"
            elif "语法错误" in error_msg:
                return "SQL语法验证阻止" 
            elif "权限不足" in error_msg:
                return "权限控制阻止"
            elif "当前事务被终止" in error_msg:
                return "事务控制阻止"
            elif "表不存在" in error_msg:
                return "对象访问控制阻止"
            else:
                return "其他安全机制阻止"
                ...
                try:
                    ...
                    try:
                        ...
                        results.append({
                            ...
                            "pattern": pattern,
                            "safe_query": "通过",
                            "message": "参数化查询正确处理了注入",
                            "block_type": classify_error(str(e))
                        })
        ...
        return {
            ...
            "summary": {
                ...
                "security_score": f"{(sum(1 for r in results if r.get('safe_query') == '通过' or r.get('unsafe_query') == '安全')/len(results))*100:.1f}%",
                "block_types": block_type_stats
            },
            "recommendations": [
                "所有数据库查询都应使用参数化查询",
                "避免直接拼接SQL字符串",
