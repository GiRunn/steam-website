...
const Homepage = () => {
  ...
  const FeaturesContainer = ({ features }) => {
    ...
    useEffect(() => {
        ...
        (entries) => {
          entries.forEach((entry) => {
            if (entry.isIntersecting) {
              ...
              setVisibleFeatures(prev => ({ ...prev, [featureId]: true }));
            }
          });
        },
        { threshold: 0.2 }
      ...
      document.querySelectorAll('[data-feature-id]').forEach(el => observer.observe(el));
        ...
        {features.map((feature, index) => (
          <div
            ...
            className={`
              ...
              ${visibleFeatures[feature.id] ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}
            `}
          >
            ...
            <div className={`relative group ${index % 2 !== 0 && 'lg:order-1'}`}>
              <div className="relative rounded-xl overflow-hidden transform hover:scale-105 transition-all duration-700">
                <img 
                  src={feature.image}
                  alt={feature.title}
              ...
              <div className={`absolute -bottom-6 ${index % 2 === 0 ? '-right-6' : '-left-6'}
  ...
  // 在 FeaturesContainer 组件定义之后添加
    const FeatureSection = ({ feature, index, inView }) => {
    const sectionId = `feature-${feature.id}`;
    const isAnimated = hasAnimated[sectionId];
    const isEven = index % 2 === 0;
  
    return (
      <div className={`
        grid grid-cols-1 lg:grid-cols-2 gap-12 items-center
        opacity-0 transform transition-all duration-1000
