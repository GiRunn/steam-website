// server/src/modules/gameOverview/repositories/gameOverviewRepository.ts
import { Pool } from 'pg';
import { Redis } from 'ioredis';

export class GameOverviewRepository {
    constructor(
        private readonly pool: Pool,
        private readonly redis: Redis
    ) {}

    async getGameOverview(gameId: number) {
        // 首先尝试从Redis缓存获取
...
// server/src/modules/gameOverview/services/gameOverviewService.ts
import { GameOverviewRepository } from '../repositories/gameOverviewRepository';

export class GameOverviewService {
    constructor(private readonly gameOverviewRepository: GameOverviewRepository) {}
...
// server/src/modules/gameOverview/controllers/gameOverviewController.ts
import { Request, Response } from 'express';
import { GameOverviewService } from '../services/gameOverviewService';

export class GameOverviewController {
    constructor(private readonly gameOverviewService: GameOverviewService) {}
...
// server/src/modules/gameOverview/routes/gameOverviewRoutes.ts
import { Router } from 'express';
import { GameOverviewController } from '../controllers/gameOverviewController';

export function setupGameOverviewRoutes(gameOverviewController: GameOverviewController): Router {
    const router = Router();
...
// server/src/modules/gameOverview/interfaces/gameOverview.interface.ts
