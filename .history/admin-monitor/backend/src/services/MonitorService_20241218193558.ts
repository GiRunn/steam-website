                ...
                SELECT json_build_object(
                    'cpu_usage', (
                        SELECT ROUND(COALESCE(
                            (SELECT (SELECT EXTRACT(epoch FROM (now() - backend_start)) 
                             FROM pg_stat_activity WHERE pid = pg_backend_pid()) * 100.0 /
                            (SELECT EXTRACT(epoch FROM (now() - pg_postmaster_start_time()))), 0
                        ), 2)
                    ),
                    'memory_usage', (
                        SELECT ROUND(COALESCE(
                            (SELECT SUM(pg_size_bytes(setting)) * 100.0 / 
                             (SELECT pg_size_bytes(setting) FROM pg_settings WHERE name = 'shared_buffers')
                             FROM pg_settings 
                             WHERE name IN ('work_mem', 'maintenance_work_mem')), 0
                        ), 2)
                    ),
                    'database_size', (
            ...
            const result = await client.query(`
                ...
                index_stats AS (
                    SELECT 
                        ...
                        idx_tup_read,
                        idx_tup_fetch,
                        CASE WHEN idx_scan = 0 THEN 'Unused index'
