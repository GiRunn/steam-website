...
class SecurityTester:
    ...
    def test_sql_injection(self, conn: psycopg2.extensions.connection) -> Dict[str, Any]:
                ...
                try:
                    ...
                    try:
                        if "ORDER BY" in test_query["safe_query"]:
                            cursor.execute(sql.SQL(test_query["safe_query"]).format(
                        ...
                        # 查询执行成功但没有返回结果，说明参数化查询正常工作
                        results.append({
                            "test_name": test_query["name"],
                            "pattern": pattern,
                            "safe_query": "通过",
                            "message": "参数化查询正确处理了注入尝试",
                            "error_type": "正常阻止",  # 新增错误类型标记
                            "blocked_by": "参数化查询"  # 新增阻止方式标记
                        })
                    except psycopg2.Error as e:
                        if "invalid input syntax" in str(e):
                            ...
                            results.append({
                                ...
                                "pattern": pattern,
                                "safe_query": "通过",
                                "message": "数据库类型系统阻止了注入尝试",
                                "error_type": "类型错误",  # 新增错误类型标记
                                "blocked_by": "数据库类型检查"  # 新增阻止方式标记
                            })
                            ...
                            results.append({
                                ...
                                "pattern": pattern,
                                "safe_query": "通过",
                                "message": "数据库权限系统阻止了注入尝试",
                                "error_type": "权限错误",  # 新增错误类型标记
                                "blocked_by": "权限检查"  # 新增阻止方式标记
                            })
                        else:
                            # 其他数据库错误
                            results.append({
                                ...
                                "safe_query": "失败",
                                "error": str(e),
                                "error_type": "数据库错误",  # 新增错误类型标记
                                "blocked_by": "其他数据库机制"  # 新增阻止方式标记
                            })
                    ...
                    try:
                        ...
                        results.append({
                            ...
                            "error_type": "注入成功",  # 新增错误类型标记
                            "blocked_by": "无"  # 新增阻止方式标记
                        })
                    except psycopg2.Error as e:
                        if "syntax error" in str(e):
                            ...
                            results.append({
                                ...
                                "pattern": pattern,
                                "unsafe_query": "安全",
                                "message": "SQL语法检查阻止了注入尝试",
                                "error_type": "语法错误",  # 新增错误类型标记
                                "blocked_by": "SQL语法检查"  # 新增阻止方式标记
                            })
                        elif "permission denied" in str(e):
                            results.append({
                                "test_name": test_query["name"],
                    ...
                    results.append({
                        ...
                        "status": "错误",
                        "error": str(e),
                        "error_type": "测试代码错误",  # 新增错误类型标记
                        "blocked_by": "测试框架"  # 新增阻止方式标记
                    })
        ...
        for r in results:
            if "blocked_by" in r:
                blocking_stats[r["blocked_by"]] = blocking_stats.get(r["blocked_by"], 0) + 1
        ...
        return {
            ...
            "summary": {
                ...
                "blocking_methods": blocking_stats  # 新增阻止方式统计
            },
            "recommendations": [
                "所有数据库查询都应使用参数化查询",
                "避免直接拼接SQL字符串",
