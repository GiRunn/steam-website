...
-- 创建测试结果临时表（用于存储详细测试数据）
CREATE TEMP TABLE IF NOT EXISTS temp_test_metrics (
...
BEGIN
    ...
    BEGIN
        ...
        PERFORM review_system.run_test(
            ...
            FROM review_system.run_stress_test(
                10000,              -- 10000个并发用户
                100000,            -- 每用户100000次操作
        ...
        PERFORM review_system.run_test(
            ...
            '安全测试'::TEXT,
            $sql$
            SELECT review_system.simulate_ddos_attack(
                50000,             -- 每秒请求数
    ...
    BEGIN
        ...
        PERFORM review_system.run_test(
            ...
            '安全测试'::TEXT,
            $sql$
            SELECT review_system.test_sql_injection(
                ARRAY[
                    'DROP TABLE reviews; --',
                    ''' OR ''1''=''1',
        ...
        PERFORM review_system.run_test(
            ...
            '安全测试'::TEXT,
            $sql$
            SELECT review_system.test_xss_protection(
                ARRAY[
        ...
        PERFORM review_system.run_test(
            ...
            '安全测试'::TEXT,
            $sql$
            SELECT review_system.test_privilege_escalation();
            $sql$::TEXT
        );
    EXCEPTION WHEN OTHERS THEN
        v_error_count := v_error_count + 1;
    ...
    BEGIN
        ...
        PERFORM review_system.run_test(
            ...
            '极限测试'::TEXT,
            $sql$
            SELECT review_system.test_massive_data_load(
                10000000,  -- 1000万条记录
                1000      -- 每批次1000条
            );
            $sql$::TEXT
        ...
        PERFORM review_system.run_test(
            ...
            '极限测试'::TEXT,
            $sql$
            SELECT review_system.test_long_content(
                10 * 1024 * 1024  -- 10MB文本
            );
            $sql$::TEXT
        );
        ...
        PERFORM review_system.run_test(
            ...
            '极限测试'::TEXT,
            $sql$
            SELECT review_system.test_special_characters(
                ARRAY[
                    E'\u0000',  -- NULL字符
                    E'\u001F',  -- 控制字符
                    E'\uFFFD',  -- 替换字符
    ...
    BEGIN
        ...
        PERFORM review_system.run_test(
            ...
            '性能测试'::TEXT,
            $sql$
            SELECT review_system.test_complex_query_performance(
                1000,    -- 1000次迭代
                TRUE     -- 启用查询计划分析
            );
            $sql$::TEXT
        ...
        PERFORM review_system.run_test(
            ...
            '性能测试'::TEXT,
            $sql$
            SELECT review_system.test_concurrent_updates(
                5000,    -- 5000个并发事务
                10000    -- 每个事务10000次更新
            );
            $sql$::TEXT
        ...
        PERFORM review_system.run_test(
            ...
            '性能测试'::TEXT,
            $sql$
            SELECT review_system.test_partition_performance(
                100,     -- 100个分区
    ...
    BEGIN
        ...
        PERFORM review_system.run_test(
            ...
            '故障恢复测试'::TEXT,
            $sql$
            SELECT review_system.test_disk_failure_recovery();
            $sql$::TEXT
        );
        ...
        PERFORM review_system.run_test(
            ...
            '故障恢复测试'::TEXT,
            $sql$
            SELECT review_system.test_network_failure_recovery();
            $sql$::TEXT
        );
        ...
        PERFORM review_system.run_test(
            ...
            '故障恢复测试'::TEXT,
            $sql$
            SELECT review_system.test_process_crash_recovery();
            $sql$::TEXT
        );
    EXCEPTION WHEN OTHERS THEN
        v_error_count := v_error_count + 1;
