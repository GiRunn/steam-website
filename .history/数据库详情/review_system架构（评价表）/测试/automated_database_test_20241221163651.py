...
class DatabaseTestFramework:
    ...
    def _test_sql_injection_attacks(self, conn: psycopg2.extensions.connection) -> Dict[str, Any]:
        cursor = conn.cursor()
        results = []
        query_stats = None  # 初始化query_stats变量
        
        try:
            # 开始新的事务
            conn.rollback()  # 确保清除任何之前失败的事务状态
            
            # 尝试使用pg_stat_statements，如果失败则继续
            try:
                cursor.execute("""
                    SELECT COUNT(*) 
                    FROM pg_extension 
                ...
                if cursor.fetchone()[0] == 0:
                    self.logger.info("pg_stat_statements扩展未安装，跳过性能统计")
                else:
                    # 尝试获取查询统计
                    cursor.execute("""
                ...
                self.logger.warning(f"性能统计功能不可用: {str(e)}")
            ...
            for test_case in injection_patterns:
                try:
                    ...
                    results.append({
                        ...
                        "status": "需加固" if count > 0 else "安全",
                        "message": f"查询返回 {count} 条结果"
                    })
                    
                    conn.commit()  # 提交每个测试的事务
            ...
            return {
                ...
                "details": {
                    "测试说明": "模拟各种SQL注入攻击",
                    "攻击类型": [p["name"] for p in injection_patterns],
                    "测试结果": results,
                    "性能统计": "已完成" if query_stats else "未启用"
                }
            }
            
        except Exception as e:
            conn.rollback()
            self.logger.error(f"SQL注入测试失败: {str(e)}")
            return {
                "test": "SQL注入攻击测试",
                "status": "错误",
                "error": str(e),
        ...
        try:
            ...
            return {
                ...
                "details": {
                    ...
                    "恢复状态": {
                        "数据一致性": "正常" if recovered_count == 0 else "异",
    ...
    # 在DatabaseTestFramework类中加_delayed_query方法
    ...
    def monitor_database_sessions(self, conn: psycopg2.extensions.connection) -> None:
        """监控数据库会话状态"""
        cursor = conn.cursor()
        try:
            cursor.execute("""
            ...
            self.logger.info(f"当前活动会话数: {len(active_sessions)}")
            ...
            self.logger.error(f"监控数据库会话失败: {str(e)}")
...
class SecurityTester:
    ...
    def test_sql_injection(self, conn: psycopg2.extensions.connection) -> Dict[str, Any]:
        ...
        injection_patterns = [
            ...
            {
                "name": "堆叠查询注入",
                "pattern": "'; DROP TABLE review_system.reviews_partitioned; --",
                "description": "测试多语句执行"
            },
            {
                "name": "时间盲注",
