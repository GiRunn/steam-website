...
const FeaturesContainer = ({ features }) => {
  ...
  const animatedFeatures = useRef(new Set());
  ...
  useEffect(() => {
    ...
    features.forEach(feature => {
      initialVisibility[feature.id] = false;
    });
    setVisibleFeatures(initialVisibility);
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          ...
          if (featureId) {
            // 只有在元素进入视口且未播放过动画时才触发
            if (entry.isIntersecting && !animatedFeatures.current.has(featureId)) {
              setVisibleFeatures(prev => ({
                ...prev,
                [featureId]: true
              }));
              // 记录该特性已播放过动画
              animatedFeatures.current.add(featureId);
            }
          }
        });
      },
    ...
    <div ref={containerRef} className="mt-20 space-y-20">
      {features.map((feature, index) => (
        <FeatureSection
          ...
          feature={feature}
          index={index}
          isVisible={visibleFeatures[feature.id]}
        />
      ))}
    </div>
  );
...
const FeatureSection = ({ feature, index, isVisible }) => {
  const isEven = index % 2 === 0;
  
  // 添加持久化的动画类
  const animationClass = isVisible ? 'animate-feature-in' : '';
    ...
    >
      ...
      <div className={`space-y-6 ${!isEven && 'lg:order-2'}`}>
        <div className="relative">
          <div className={`
            inline-block rounded-lg bg-gradient-to-r from-blue-500/20 to-purple-500/20 p-2 
        ...
        <h3 className={`
          text-3xl font-bold text-white group-hover:text-transparent 
          bg-clip-text bg-gradient-to-r from-blue-400 to-purple-400
          ${isVisible ? 'animate-fade-slide-in delay-100' : ''}
        `}>
          {feature.title}
        </h3>
        ...
        <p className={`
          ...
          ${isVisible ? 'animate-fade-slide-in delay-200' : ''}
        `}>
          {feature.description}
        </p>
          ...
          {feature.bulletPoints.map((point, idx) => (
            <li 
              ...
              className={`
                ...
                ${isVisible ? `animate-fade-slide-in delay-${300 + idx * 100}` : ''}
              `}
            >
      ...
      <div className={`relative group ${!isEven && 'lg:order-1'}`}>
        <div className={`
          ...
          ${isVisible ? 'animate-fade-scale-in delay-300' : ''}
        `}>
          <img 
            src={feature.image}
            alt={feature.title}
        ...
        <div className={`
          absolute -bottom-6 ${isEven ? '-right-6' : '-left-6'}
          ...
          ${isVisible ? 'animate-fade-slide-up delay-400' : ''}
        `}>
...
useEffect(() => {
 ...
 // 在 state 声明处添加新的 state
const [hasAnimated, setHasAnimated] = useState({});

// 修改后的 IntersectionObserver
useEffect(() => {
  const observers = [];
  const observerCallback = (elementId) => (entries) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting && !hasAnimated[elementId]) {
        setHasAnimated(prev => ({
          ...prev,
          [elementId]: true
        }));
  ...
  // 创建观察器
  const createObserver = (elementId, threshold = 0.1) => {
    return new IntersectionObserver(observerCallback(elementId), {
      threshold,
      rootMargin: '50px' // 提前触发以使动画更流畅
    });
  };

  // 观察产品特性区域
  if (introRef.current) {
    const observer = createObserver('features-section');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#1b2838] via-[#1b2838] to-[#2a475e]">
      ...
      <div className="relative py-20 overflow-hidden" 
        style={{ perspective: '1000px' }} // 添加3D视角效果
      >
        {/* 背景动效 */}
        ...
        <div className="relative container mx-auto px-4">
          <div className="max-w-7xl mx-auto">
            {/* 标题部分 - 增强动画效果 */}
            <div className="text-center mb-16 space-y-4 opacity-0 animate-fade-in-down"
