...
const RatingBadge = ({ rating }) => (
  <motion.div
    initial={{ scale: 0 }}
    animate={{ scale: 1 }}
    className="absolute top-4 right-4 flex items-center space-x-1 bg-gradient-to-r 
...
const GameProduct = ({ product }) => {
  return (
    <motion.div
      ...
      className="bg-[#111827] rounded-xl shadow-xl overflow-hidden border border-gray-800 
                 hover:shadow-2xl transition-shadow duration-300 max-w-6xl mx-auto"
    >
      <div className="flex flex-col lg:flex-row">
        {/* 游戏图片区域 - 调整大小并优化显示 */}
        <motion.div 
          className="relative w-full lg:w-[480px] flex items-center justify-center bg-[#0a0f16] overflow-hidden group"
          whileHover={{ scale: 1.01 }}
          transition={{ duration: 0.3 }}
        >
          <div className="relative w-full pt-[75%]"> {/* 4:3 比例，更适合游戏图片显示 */}
            <motion.img
              src={product.image}
              alt={product.name}
        ...
        <div className="flex-1 p-8 lg:p-10">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            <motion.div initial={{ x: -20 }} animate={{ x: 0 }}>
              <h3 className="text-2xl lg:text-3xl font-bold text-white bg-clip-text text-transparent 
                           bg-gradient-to-r from-blue-400 to-blue-600">
                {product.name}
              </h3>
              <p className="text-gray-400 mt-3 leading-relaxed text-base lg:text-lg">
                {product.description}
              </p>
            </motion.div>
            ...
            <motion.div 
              ...
              animate={{ x: 0 }}
            >
              <div className="text-3xl lg:text-4xl font-bold text-white">${product.price}</div>
              <div className="text-sm lg:text-base text-gray-400 mt-1">含税价</div>
          ...
          >
            <div className="text-base text-gray-400 mb-3">游戏类型</div>
          ...
          {/* 底部提示信息 */}
          <motion.div 
            className="mt-8 text-sm lg:text-base text-gray-400 bg-[#1a2234] p-4 rounded-lg"
