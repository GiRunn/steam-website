...
import { ChevronLeft, ChevronRight, Search, Moon, Sun, ArrowUp, Globe } from 'lucide-react';
...
const Homepage = () => {
  ...
  const FeaturesContainer = ({ features }) => (
    <div className="mt-20 space-y-20">
      {features.map((feature, index) => (
        <div
          key={feature.id}
          className={`opacity-0 animate-fade-in-up ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}`}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#1b2838] to-[#2a475e]">
      <nav className={`sticky top-0 z-50 ${scrollDirection === 'up' ? 'translate-y-0' : scrollY > 100 ? '-translate-y-full' : 'translate-y-0'}`}>
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between h-16">
            <span className="text-xl font-bold text-white">GAME STORE</span>
      ...
      <div className="relative py-20" ref={introRef}>
        <FeaturesContainer features={featureData.features} />
      </div>

      {showScrollTop && (
        <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="fixed bottom-8 right-8 bg-blue-500 text-white p-3 rounded-full">
