 ...
 * @returns {Array} - 游戏列表
 */
const getActivityGames = (activityId) => {
 ...
 * @param {number} gameId - 游戏ID
 * @param {number} activityId - 活动ID
 * @returns {boolean} - 是否参与活动
 */
const isGameInActivity = (gameId, activityId) => {
 ...
 */
const getGameActivityDiscount = (gameId, activityId) => {
  const activities = GAME_ACTIVITIES[gameId] || [];
