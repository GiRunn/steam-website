...
const ActivityCard = ({ activity, onClick }) => {
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
      className={`relative rounded-[32px] overflow-hidden cursor-pointer bg-[#1c2127] border ${statusStyle.borderColor} 
        bg-gradient-to-br ${statusStyle.cardBackground}`}
      onClick={onClick}
    >
      <div className={`absolute -inset-0.5 ${statusStyle.hoverBorder} rounded-[32px] opacity-0 hover:opacity-100 transition-opacity duration-300`} />
      ...
      <div className="relative">
        ...
        <div className="p-6">
          ...
          <motion.button
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            className={`w-full flex items-center justify-center gap-2 px-6 py-3 rounded-lg ${statusStyle.buttonBg} text-white font-medium transition-colors`}
          >
            查看详情
