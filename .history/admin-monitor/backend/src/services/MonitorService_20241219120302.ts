        ...
        try {
            const result = await client.query(`
                SELECT json_build_object(
                    ...
                    'avg_review_length', (
                        SELECT COALESCE(ROUND(AVG(LENGTH(content))::numeric, 2), 0) 
                        FROM reviews
                    )
                ) as metrics;
            `);
            ...
            const result = await client.query(`
                SELECT json_build_object(
                    'partitions', (
                        ...
                        FROM pg_stat_user_tables
                        WHERE relname LIKE 'reviews_%'
                        ORDER BY relname
                    )
                ) as partition_info;
            `);

            const partitionInfo = result.rows[0].partition_info;
            return {
                partitions: partitionInfo.partitions || [],
                message: partitionInfo.partitions?.length > 0 ? 'success' : '暂无分区数据'
