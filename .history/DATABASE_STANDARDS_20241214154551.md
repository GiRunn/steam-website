...
INSERT INTO review_system.reviews_partitioned (
    ...
    is_recommended, platform, language, created_at
) VALUES 
(1001, 1, 4.5, '非常好玩的游戏！画面精美，剧情丰富。', 10, true, 'PC', 'zh-CN', CURRENT_TIMESTAMP),
(1001, 2, 4.0, 'Great game but needs improvements.', 15, true, 'PS5', 'en-US', CURRENT_TIMESTAMP),
(1001, 3, 4.8, 'とても面白いゲームです！', 12, true, 'PS5', 'ja-JP', CURRENT_TIMESTAMP);
...
(1001, 6, 4.2, 'Next month review', 25, true, 'PC', 'en-US', 
...
BEGIN;
    ...
    WHERE game_id IN (1001, 1002, 1003);
...
-- 子分区
table_name_y{year}m{month}  -- 按月分区
table_name_y{year}q{quarter} -- 按季度分区
table_name_p{number}        -- 按数值范围分区
```
...
-- 权限分配测试
GRANT SELECT ON ALL TABLES IN SCHEMA public TO app_readonly;
GRANT SELECT, INSERT, UPDATE ON specific_table TO app_readwrite;
```
...
BEGIN
    -- 测试用例
    ASSERT (SELECT count(*) FROM table) > 0, '表不能为空';
    -- 更多测试...
END;
$$;
```
