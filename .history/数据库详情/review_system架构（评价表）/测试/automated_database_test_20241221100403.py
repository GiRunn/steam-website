    ...
    def test_review_summary_update(self, conn: psycopg2.extensions.connection) -> Dict[str, Any]:
        ...
        try:
            ...
            test_data = [
                ...
                # 游戏3的评论
                (1003, 1, Decimal('5.0'), '年度最佳游戏！', 30, 'PC', 'zh-CN', True),
                (1003, 2, Decimal('4.9'), 'Best game ever!', 25, 'PS5', 'en-US', True),
                (1003, 3, Decimal('4.8'), '最高のゲーム体験', 20, 'PC', 'ja-JP', True)
            ]
            ...
            if summary:
                (total_reviews, avg_rating, total_playtime, 
                 pc_count, ps5_count, xbox_count,
                 zh_cn_count, en_us_count, ja_jp_count, es_es_count,
                 recommended_count) = summary
                
                # 游戏1的预期结果
                expected = {
                    "total_reviews": 5,
                    "average_rating": Decimal('4.36'),  # (4.5 + 3.5 + 5.0 + 4.0 + 4.8) / 5
                    "total_playtime": 58,  # 10 + 5 + 15 + 8 + 20
                    "pc_count": 3,
                    "ps5_count": 1,
                    "xbox_count": 1,
                    "zh_cn_count": 2,
                ...
                results.extend([
                    {
                        "test": "评论总数",
                        "status": "通过" if total_reviews == expected["total_reviews"] else "失败",
                        "expected": expected["total_reviews"],
                        "actual": total_reviews
                    },
                    {
                        "test": "平均评分",
                        "status": "通过" if abs(avg_rating - expected["average_rating"]) < Decimal('0.01') else "失败",
                        "expected": float(expected["average_rating"]),
                        "actual": float(avg_rating)
                    },
                    {
                        "test": "总游戏时间",
                        "status": "通过" if total_playtime == expected["total_playtime"] else "失败",
                        "expected": expected["total_playtime"],
                        "actual": total_playtime
                    },
                    {
                        "test": "PC平台数量",
                        "status": "通过" if pc_count == expected["pc_count"] else "失败",
                        "expected": expected["pc_count"],
                        "actual": pc_count
                    },
                    {
                        "test": "PS5平台数量",
                        "status": "通过" if ps5_count == expected["ps5_count"] else "失败",
                        "expected": expected["ps5_count"],
                        "actual": ps5_count
                    },
                    {
                        "test": "推荐数量",
                        "status": "通过" if recommended_count == expected["recommended_count"] else "失败",
                        "expected": expected["recommended_count"],
                        "actual": recommended_count
                    }
                ])

            # 检查游戏2和游戏3的基本数据
            cursor.execute("""
                SELECT game_id, total_reviews, average_rating
                FROM review_system.review_summary_partitioned
                WHERE game_id IN (1002, 1003)
                ORDER BY game_id
            """)
            
            for game_summary in cursor.fetchall():
                game_id, total_reviews, avg_rating = game_summary
                if game_id == 1002:
                    expected_total = 3
                    expected_avg = Decimal('3.23')  # (3.0 + 4.2 + 2.5) / 3
                else:  # game_id == 1003
                    expected_total = 3
                    expected_avg = Decimal('4.90')  # (5.0 + 4.9 + 4.8) / 3
                    
                results.extend([
                    {
                        "test": f"游戏{game_id}评论总数",
                        "status": "通过" if total_reviews == expected_total else "失败",
                        "expected": expected_total,
                        "actual": total_reviews
                    },
                    {
                        "test": f"游戏{game_id}平均评分",
    ...
    def _deadlock_test_task(self, conn: psycopg2.extensions.connection, review_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            cursor.execute("""
                ...
                VALUES (%s, %s, %s)
            """, (review_id, random.randint(1, 1000), "死锁测试���复"))
