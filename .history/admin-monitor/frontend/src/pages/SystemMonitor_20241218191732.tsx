...
const SystemMonitor: React.FC = () => {
    ...
    const fetchMetrics = useCallback(async () => {
        try {
            ...
            setMetrics({
                system: [
                    { 
                        ...
                        unit: '%', 
                        trend: getTrend(systemData.cpu_usage, metricsHistory.cpu) 
                    },
                    { 
                        name: '内存使用', 
                        value: systemData.memory_usage, 
                        unit: '%', 
                        trend: getTrend(systemData.memory_usage, metricsHistory.memory) 
                    }
                ],
                database: [
                    { 
                        name: '数据库大小', 
                        value: databaseData.size, 
                        unit: 'MB', 
                        trend: 'up' 
                    },
                    { 
                        name: '活跃连接', 
                        value: databaseData.active_connections, 
                        trend: getTrend(databaseData.active_connections, metricsHistory.connections) 
                    }
                ],
                performance: [
                    { 
                        name: '缓存命中率', 
                        value: performanceData.cache_hit_ratio, 
                        unit: '%', 
                        trend: getTrend(performanceData.cache_hit_ratio, metricsHistory.cache) 
                    },
                    { 
                        name: '每小时事务数', 
                        value: performanceData.transactions_per_hour, 
                        trend: getTrend(performanceData.transactions_per_hour, metricsHistory.transactions) 
                    }
                ]
            });
            ...
            setMetricsHistory(prev => ({
                ...
                connections: [...prev.connections.slice(-20), { time: timestamp, value: databaseData.active_connections }],
                transactions: [...prev.transactions.slice(-20), { time: timestamp, value: performanceData.transactions_per_hour }],
                cache: [...prev.cache.slice(-20), { time: timestamp, value: performanceData.cache_hit_ratio }]
            }));
    ...
    // 添加趋势判断函数
    const getTrend = (currentValue: number, history: any[]): 'up' | 'down' | 'stable' => {
